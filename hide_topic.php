...
// Verify topic
$stmt = $conn->prepare("SELECT hospital_id, health_center_id FROM topics WHERE id = ? AND status = 'visible'");
...
if (!$topic || ($hospital_id && $topic['hospital_id'] != $user['hospital_id']) || ($health_center_id && $topic['health_center_id'] != $user['health_center_id'])) {
    $_SESSION['error'] = "You are not authorized to hide this topic.";
...
// Hide topic
$stmt = $conn->prepare("UPDATE topics SET status = 'hidden' WHERE id = ?");
$stmt->bind_param("i", $topic_id);
if ($stmt->execute()) {
    $_SESSION['success'] = "Topic hidden successfully.";
} else {
    $_SESSION['error'] = "Failed to hide topic: " . $conn->error;
}
$stmt->close();
